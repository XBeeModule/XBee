...
__attribute__((always_inline)) void UTFT::drawHLine(int x, int y, int l, uint16_t color) {
    if (color == VGA_NODEF) color = _fgc;
	if (l < 0) {
		l = -l;
		x -= l;
	}
...
__attribute__((always_inline)) void UTFT::drawHLine_noCS(int x, int y, int l, uint16_t color) {
    if (color == VGA_NODEF) color = _fgc;
	if (l < 0) {
		l = -l;
		x -= l;
...
void UTFT::drawVLine(int x, int y, int l, uint16_t color) {
    if (color == VGA_NODEF) color = _fgc;
	if (l<0) {
		l = -l;
		y -= l;
...
__attribute__((always_inline)) void UTFT::drawVLine_noCS(int x, int y, int l, uint16_t color) {
    if (color == VGA_NODEF) color = _fgc;
	if (l<0) {
		l = -l;
		y -= l;
...
void UTFT::drawRect(int x1, int y1, int x2, int y2, uint16_t color) {
    if (color == VGA_NODEF) color = _fgc;
	if (x1>x2)	{
		swap(int, x1, x2);
	}
...
__attribute__((always_inline)) void UTFT::drawPixel(int x, int y, uint16_t color) {
    if (color == VGA_NODEF) color = _fgc;
	TFT_CS_LOW;
	setXY(x, y, x, y);
	wr_comm_last(RAMWR);
...
__attribute__((always_inline)) void UTFT::drawPixel_noCS(int x, int y, uint16_t color) {
    if (color == VGA_NODEF) color = _fgc;
	setXY(x, y, x, y);
	wr_comm_last(RAMWR);
	uint16_t fg = (uint8_t(color & 0xFF) <<8 )|( uint8_t(color>>8));
...
void UTFT::drawLine(int x1, int y1, int x2, int y2, uint16_t color) {
...
uint16_t UTFT::getColor(uint8_t r, uint8_t g, uint8_t b) {
...
void UTFT::drawRoundRect(int x1, int y1, int x2, int y2, uint16_t color) {
    if (color == VGA_NODEF) color = _fgc;
    if (x1>x2) {
        swap(int, x1, x2);
    }
...
void UTFT::fillRoundRect(int x1, int y1, int x2, int y2, uint16_t color) {
...
void UTFT::drawCircle(int x, int y, int radius, uint16_t color) {
...
__attribute__((always_inline)) void UTFT::fillCircle(int x, int y, int radius, uint16_t color) {
...
__attribute__((always_inline)) void UTFT::setPixel(uint16_t color) {
...
__attribute__((always_inline)) void UTFT::print(String st, int x, int y, int deg) {
...
__attribute__((always_inline)) void UTFT::print(String st) {
    char buf[st.length()+1];
    st.toCharArray(buf, st.length()+1);
    print(buf);
}

__attribute__((always_inline)) void UTFT::println(char *st) {
    print(st);
    _y += cfont.y_size + 1;
    _x = _xmin;
}

__attribute__((always_inline)) void UTFT::println(String st) {
	char buf[st.length()+1];
    st.toCharArray(buf, st.length()+1);
	println(buf);
}

__attribute__((always_inline)) void  UTFT::println(long n) {
  char buf[25]; 	
  sprintf(buf, "%ld", n);
  println(buf);
};
...
void UTFT::setTextArea(int x, int y, int width, int height) {
	_x = x;
	_y = y;
	_xmin = x;
	_ymin = y;
...
/*
void UTFT::readRect(int16_t x, int16_t y, int16_t w, int16_t h, uint8_t *pcolors)
{
	uint16_t c = w * h;
	uint8_t r,g,b;
...
void UTFT::writeRect(int16_t x, int16_t y, int16_t w, int16_t h, const uint8_t *pcolors)
{
    uint16_t c = w * h;
	TFT_CS_LOW;    
	setXY(x, y, x+w-1, y+h-1);
