...
// скопировать общие поля из ответа XBee целевой ответ
...
void XBeeResponse::getZBTxStatusResponse(XBeeResponse &zbXBeeResponse) // XBee статус ответа
{

	// way off?
	ZBTxStatusResponse* zb = static_cast<ZBTxStatusResponse*>(&zbXBeeResponse);
	// pass pointer array to subclass
	// пройти массив указателей на подкласса
...
// маркеры для считывания данных из массива пакета. это индекс, так что 12-й элемент массива
...
void XBeeResponse::getZBRxResponse(XBeeResponse &rxResponse) 
{
	...
	//TODO проверить ответ API ID Совпадение этот API для этого ответ

	// передать массив указателей на подкласса
	zb->setFrameData(getFrameData());
...
void XBeeResponse::getRemoteAtCommandResponse(XBeeResponse &response) 
{
	...
	// нет реальной необходимости, чтобы бросить. изменить параметр для соответствует ожидаемому класс
...
// как преобразовать ответ на соответствующую подкласса?
...
void XBee::readPacket()
{
	...
	{
		// отказаться от предыдущего пакета и начать все сначала
		resetResponse();
	}
    ...
    while (available())
	{
		...
		{
        	// Новый старт пакет до предыдущая пакетирован в эксплуатацию - отбросить предыдущий пакет и начать все сначала
		...
		if (_pos > 0 && b == ESCAPE) 
		{
			...
			{
				// избежать байт. Следующий байт будет
				_escape = true;
				continue;
			}
		}
		...
		// Контрольная сумма включает все байты, начиная с api id
        ...
        switch(_pos) 
		{
			...
			default:
				...
				{
					// превышает максимальный размер. никогда не должны происходить
