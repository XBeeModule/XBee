...
uint16_t UTFT::color565(uint8_t r, uint8_t g, uint8_t b) {
    return ((r & 0xF8) << 8) | ((g & 0xFC) << 3) | ((b & 0xF8) >> 3);
}

UTFT::UTFT(int8_t cs, int8_t dc, int8_t mosi,
...
UTFT::UTFT(int8_t cs, int8_t dc, int8_t rst) : Adafruit_GFX(ILI9341_TFTWIDTH, ILI9341_TFTHEIGHT) {
    _cs   = cs;
    _dc   = dc;
    _rst  = rst;
    _sclk  = -1;
...
#ifdef ESP32
void UTFT::begin(uint32_t freq, SPIClass &spi)
#else
void UTFT::begin(uint32_t freq)
#endif
{
#ifdef ESP32
    _spi = spi;
...
void UTFT::invertDisplay(boolean i) {
    startWrite();
    writeCommand(i ? ILI9341_INVON : ILI9341_INVOFF);
    endWrite();
}

void UTFT::scrollTo(uint16_t y) {
    startWrite();
...
void UTFT::startWrite(void){
    SPI_BEGIN_TRANSACTION();
    SPI_CS_LOW();
}

void UTFT::endWrite(void){
    SPI_CS_HIGH();
    SPI_END_TRANSACTION();
}

void UTFT::writeCommand(uint8_t cmd){
    SPI_DC_LOW();
    spiWrite(cmd);
    SPI_DC_HIGH();
}

void UTFT::setAddrWindow(uint16_t x, uint16_t y, uint16_t w, uint16_t h) {
...
void UTFT::pushColor(uint16_t color) {
  startWrite();
  SPI_WRITE16(color);
  endWrite();
}
...
void UTFT::writePixel(uint16_t color){
    SPI_WRITE16(color);
}

void UTFT::writePixels(uint16_t * colors, uint32_t len){
    SPI_WRITE_PIXELS((uint8_t*)colors , len * 2);
}

void UTFT::writeColor(uint16_t color, uint32_t len){
#ifdef SPI_HAS_WRITE_PIXELS
    if(_sclk >= 0){
...
void UTFT::writePixel(int16_t x, int16_t y, uint16_t color) {
    if((x < 0) ||(x >= _width) || (y < 0) || (y >= _height)) return;
    setAddrWindow(x,y,1,1);
    writePixel(color);
}

void UTFT::writeFillRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t color){
...
void UTFT::writeFastVLine(int16_t x, int16_t y, int16_t h, uint16_t color){
    writeFillRect(x, y, 1, h, color);
}

void UTFT::writeFastHLine(int16_t x, int16_t y, int16_t w, uint16_t color){
...
void UTFT::drawFastVLine(int16_t x, int16_t y,
...
void UTFT::drawFastHLine(int16_t x, int16_t y,
...
void UTFT::fillRect(int16_t x, int16_t y, int16_t w, int16_t h,
        uint16_t color) {
    startWrite();
...
void UTFT::drawRGBBitmap(int16_t x, int16_t y,
