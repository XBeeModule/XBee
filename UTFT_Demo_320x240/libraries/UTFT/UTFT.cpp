...
// Include hardware-specific functions for the correct MCU
  ...
  #include "hardware/pic32/HW_PIC32.h"
	...
	#elif defined(__SAMD21E18A__) || defined(__SAMD21G18A__) || defined(__SAMD21J18A__)
		#pragma message("Compiling for SAMD21G18A LaunchPad...")
...
{ 
	...
	byte	dtm[] = {16, 16, 16, 8, 8, 16, 8, SERIAL_4PIN, 16, SERIAL_5PIN,					// 00-09
					 ...
					 SERIAL_5PIN, SERIAL_5PIN, SERIAL_4PIN};												// 30-
	...
	//
	//volatile uint32_t *setDCPin = &PORT->Group[g_APinDescription[RS].ulPort].OUTSET.reg;
	//volatile uint32_t *clrDCPin = &PORT->Group[g_APinDescription[RS].ulPort].OUTCLR.reg;
	//
	...
	//
	volatile uint32_t *setRSTPin = &PORT->Group[g_APinDescription[RST].ulPort].OUTSET.reg;
	volatile uint32_t *clrRSTPin = &PORT->Group[g_APinDescription[RST].ulPort].OUTCLR.reg;
	//
	#define TFT_CS_LOW  *clrCSPin = (1ul << g_APinDescription[CS].ulPin)
	#define TFT_CS_HIGH  *setCSPin = (1ul << g_APinDescription[CS].ulPin)
	//
	//#define TFT_DC_LOW   *clrDCPin = (1ul << g_APinDescription[RS].ulPin)
	//#define TFT_DC_HIGH  *setDCPin = (1ul << g_APinDescription[RS].ulPin)
	//
	#define TFT_RST_OFF *setRSTPin = (1ul << g_APinDescription[RST].ulPin)
	#define TFT_RST_ON  *clrRSTPin = (1ul << g_APinDescription[RST].ulPin)
	//
